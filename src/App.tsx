...
export default function App() {
  ...
  const [prompt, setPrompt] = useState(questExamples[0]);
  const [notice, setNotice] = useState("The hillside quest is ready. Direct the next beat.");
  const [activity, setActivity] = useState<Activity[]>([{ id: 0, source: "Stage", text: "Demo scene initialized: Sir Arthur is mounted and Ember is watching from the roost." }]);
