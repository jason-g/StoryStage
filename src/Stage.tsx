...
import horseArt from "./assets/bramble-marionette.webp";
...
export default function Stage({ scene, activeBeat }: Props) {
  ...
  const movement = activeBeat && ["enter", "walk", "move", "run", "hide", "exit", "ride", "fly", "fall"].includes(activeBeat.action);
  ...
  return <section className="stage-shell" style={movementStyle} aria-label={hillside ? "StoryStage hillside quest theater" : "StoryStage neon alley theater"}>
    ...
    <div className={`stage ${hillside ? "hillside-stage" : ""}`} aria-live="polite">
      ...
      {scene.actors.filter((actor) => actor.visible).map((actor) => { const carried = displayHeldBy(actor.id); const moving = actor.id === activeBeat?.actorId && ["enter", "walk", "move", "run", "ride", "fly", "fall"].includes(activeBeat.action); const picking = actor.id === activeBeat?.actorId && (activeBeat.action === "hold" || activeBeat.action === "grab"); const dropping = actor.id === activeBeat?.actorId && activeBeat.action === "drop"; return <div className="actor-slot" key={actor.id} style={positions[actor.zone] ?? positions.center}>{carried.includes("horse") && <img className="mounted-horse" src={horseArt} alt="" />}<Actor actor={actor} active={actor.id === activeBeat?.actorId} action={actor.id === activeBeat?.actorId ? activeBeat.action : undefined} />{carried.filter((id) => id !== "horse").length > 0 && <div className={`carried-items ${moving ? "moving" : ""} ${picking ? "picking" : ""} ${dropping ? "dropping" : ""}`}>{carried.filter((id) => id !== "horse").map((id) => <CarriedProp id={id} key={id} />)}</div>}{activeBeat?.actorId === actor.id && <div className="bubble">{activeBeat.dialogue || activeBeat.action.toUpperCase()}</div>}</div>; })}
      {activeBeat && ["shoot", "attack", "fall", "explode", "die"].includes(activeBeat.action) && <div className={`battle-effect effect-${activeBeat.action}`} aria-hidden="true">{activeBeat.action === "shoot" ? "➤" : activeBeat.action === "attack" ? "✦" : activeBeat.action === "die" ? "☠" : "✹"}</div>}
