...
const zoneSchema = { type: "string", enum: ["offstage_left", "left", "center", "right", "offstage_right", "lamp", "crate"] };
const actionSchema = { type: "string", enum: ["enter", "walk", "run", "point", "talk", "laugh", "gasp", "hide", "exit"] };
...
export function registerStoryStageTools(api: StoryStageToolApi): boolean {
  ...
  const tools: BrowserTool[] = [
    ...
    { name: "create_character", description: "Add a fox detective or robot actor to the shared stage.", inputSchema: object({ preset: { type: "string", enum: ["fox_detective", "robot"] }, name: { type: "string", minLength: 1, maxLength: 32 }, palette: { type: "string", enum: ["amber", "teal", "violet", "coral"] } }, ["preset", "name"]), execute: wrap(api.createCharacter) },
    { name: "create_scene", description: "Reset StoryStage to a supported scene.", inputSchema: object({ sceneId: { type: "string", enum: ["neon_alley"] } }, ["sceneId"]), execute: wrap(api.createScene) },
    ...
    { name: "direct_action", description: "Queue one atomic, supported stage action. Inspect the scene first; do not send multi-step directions.", inputSchema: object({ actorId: { type: "string" }, action: actionSchema, zone: zoneSchema, targetId: { type: "string", enum: ["lamp", "crate", "clue", "fenn", "nix"] }, dialogue: { type: "string", maxLength: 140 } }, ["actorId", "action"]), execute: wrap(api.directAction) },
