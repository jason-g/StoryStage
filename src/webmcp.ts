...
export function registerStoryStageTools(api: StoryStageToolApi): boolean {
  ...
  const tools: BrowserTool[] = [
    ...
    { name: "direct_action", description: "Queue one atomic, supported stage action. Inspect the scene first; do not send multi-step directions.", inputSchema: object({ actorId: { type: "string" }, action: actionSchema, zone: zoneSchema, targetId: { type: "string", enum: ["lamp", "crate", "clue", "fenn", "nix", "castle", "horse", "sword", "bow", "arrow", "aria", "ember"] }, dialogue: { type: "string", maxLength: 140 } }, ["actorId", "action"]), execute: wrap(api.directAction) },
